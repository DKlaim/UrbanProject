...
class Human:
    ...
    def __init__(self, name):  # __init__ - инициализатор (конструктор класса)
        # Сюда мы прописываем то, что будет создаваться при инициализации класса
        self.name = name  # name - переменная, которая является АТРИБУТом/характеристикой экземпляра класса и принадлежит только создаваемым (нинициируемым) объектам (внутри самого класса его не существует)


den = Human('Денис')  # den - объект класса Human (экземпляр класса) созданный нами и инициируемый как объект класса
print(type(den))
